...
With that in mind, we will define an AbstractClass that takes care of the download process.
...
from .utils import DateProcessor, DateFrequency, OSUtil
from .file_downloader import FileDownloader, ConnectionType
...
class AbstractParser(ABC):
    ...
    @abstractmethod
    def filename_fn(self, date: datetime, **kwargs) -> str:
        pass

    @abstractmethod
    def foldername_fn(self, date: datetime, **kwargs) -> str:
        pass

    def filename(self, date: Union[str, datetime], **kwargs) -> str:
        """Return just the filename given a date  and filename_fn"""
        date = DateProcessor.parse_date(date)
        return self.filename_fn(date, **kwargs)

    def foldername(self, date: Union[str, datetime], **kwargs) -> str:
        """Return just the foldername given a date string"""
        date = DateProcessor.parse_date(date)
        return self.foldername_fn(date, **kwargs)

    def remote_folder(self, date: Union[str, datetime], **kwargs) -> str:
        """Return just the remote folder given a date string"""
        # get the datetime
        return "/".join([self.root, self.foldername(date, **kwargs)])

    def remote_target(self, date: Union[str, datetime], **kwargs) -> str:
        """Target is composed by root / folder / filename"""
        return "/".join(
            [self.remote_folder(date, **kwargs), self.filename(date, **kwargs)]
        )
